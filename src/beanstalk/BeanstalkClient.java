...
public class BeanstalkClient {
    ...
    /**
     * will return the connection to the pool, or close the underlying socket if
     * this did not come from a pool
     */
    public void close() {
        if (this.pool == null) {
            if (this.con != null) {
     ...
     *
     * @param priority The job priority, from 0 to 2^32. Most urgent = 0, least
     * urgent = 4294967295.
     * @param delay The time the server will wait before putting the job on the
     * ready queue.
     * @param ttr The job time-to-run. The server will automatically release the
     * job after this TTR (in seconds) after a client reserves it.
     * @param data The job data.
     * @return The id of the inserted job.
     * @throws BeanstalkException If an unexpected response is received from the
     * server, or other unexpected problem occurs.
     */
     ...
     *
     * @param timeoutSeconds The number of seconds to wait for a job. Null if a
     * job should be reserved only if immediately available.
     ...
     *
     * @param job The job to release. This job must previously have been
     * reserved.
     * @param priority The new priority to assign to the released job.
     * @param delay The number of seconds the server should wait before placing
